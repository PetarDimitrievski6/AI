...
def prodolzhi_pravo(snake_dir, snake_pos, green_apples, red_apples):
    new_snake_pos = snake_pos.copy()
    head_x = snake_pos[0][0]
    head_y = snake_pos[0][1]
    f = False

    if snake_dir == 'up' and 0 <= head_y + 1 < 10 and (head_x, head_y + 1) not in red_apples \
            and (head_x, head_y + 1) not in snake_pos:
        new_snake_pos.insert(0, (head_x, head_y + 1))
        f = True

    if snake_dir == 'down' and 0 <= head_y - 1 < 10 and (head_x, head_y - 1) not in red_apples \
            and (head_x, head_y - 1) not in snake_pos:
        new_snake_pos.insert(0, (head_x, head_y - 1))
        f = True

    if snake_dir == 'right' and 0 <= head_x + 1 < 10 and (head_x + 1, head_y) not in red_apples \
            and (head_x + 1, head_y) not in snake_pos:
        new_snake_pos.insert(0, (head_x + 1, head_y))
        f = True

    if snake_dir == 'left' and 0 <= head_x - 1 < 10 and (head_x - 1, head_y) not in red_apples \
            and (head_x - 1, head_y) not in snake_pos:
        new_snake_pos.insert(0, (head_x - 1, head_y))
        f = True

    if f and new_snake_pos[0] not in green_apples:
        new_snake_pos.pop()

    return new_snake_pos, snake_dir


def svrti_desno(snake_dir, snake_pos, green_apples, red_apples):
    new_snake_pos = snake_pos.copy()
    new_snake_dir = snake_dir
    head_x = snake_pos[0][0]
    head_y = snake_pos[0][1]
    f = False

    if new_snake_dir == 'up' and 0 <= head_x + 1 < 10 and (head_x + 1, head_y) not in red_apples \
            and (head_x + 1, head_y) not in snake_pos:
        new_snake_pos.insert(0, (head_x + 1, head_y))
        new_snake_dir = 'right'
        f = True

    if new_snake_dir == 'down' and 0 <= head_x - 1 < 10 and (head_x - 1, head_y) not in red_apples \
            and (head_x - 1, head_y) not in snake_pos:
        new_snake_pos.insert(0, (head_x - 1, head_y))
        new_snake_dir = 'left'
        f = True

    if new_snake_dir == 'right' and 0 <= head_y - 1 < 10 and (head_x, head_y - 1) not in red_apples \
            and (head_x, head_y - 1) not in snake_pos:
        new_snake_pos.insert(0, (head_x, head_y - 1))
        new_snake_dir = 'down'
        f = True

    if new_snake_dir == 'left' and 0 <= head_y + 1 < 10 and (head_x, head_y + 1) not in red_apples \
            and (head_x, head_y + 1) not in snake_pos:
        new_snake_pos.insert(0, (head_x, head_y + 1))
        new_snake_dir = 'up'
        f = True
    ...
    return new_snake_pos, new_snake_dir


def svrti_levo(snake_dir, snake_pos, green_apples, red_apples):
    new_snake_pos = snake_pos.copy()
    new_snake_dir = snake_dir
    head_x = snake_pos[0][0]
    head_y = snake_pos[0][1]
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        snake_pos = list(state[1])
        snake_dir = state[0]
        green_apples = list(state[2])

        new_snake_pos, new_snake_dir = prodolzhi_pravo(snake_dir, snake_pos, green_apples, self.red_apples)
        if new_snake_pos != snake_pos:
            successors["ProdolzhiPravo"] = (
                new_snake_dir, tuple(new_snake_pos),
                tuple([apple for apple in green_apples if new_snake_pos[0] != apple]))

        new_snake_pos, new_snake_dir = svrti_desno(snake_dir, snake_pos, green_apples, self.red_apples)
        if new_snake_pos != snake_pos:
            successors["SvrtiDesno"] = (
                new_snake_dir, tuple(new_snake_pos),
                tuple([apple for apple in green_apples if new_snake_pos[0] != apple]))

        new_snake_pos, new_snake_dir = svrti_levo(snake_dir, snake_pos, green_apples, self.red_apples)
        if new_snake_pos != snake_pos:
...
if __name__ == '__main__':
    ...
    for i in range(int(input())):
        ...
        list_zeleni.append((int(input_str[0]), int(input_str[1])))

    list_crveni = []
    for i in range(int(input())):
        input_str = input().split(",")
        list_crveni.append((int(input_str[0]), int(input_str[1])))
    ...
    snake = Snake(list_crveni, (direction, snake_position, tuple(list_zeleni)))
