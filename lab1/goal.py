...
class Player(Problem):
    grid_width = 8
    grid_height = 6
    opponents_player = [[3, 3], [4, 5]]
    opponents_football = [[2, 2], [2, 3], [2, 4], [3, 2], [3, 3], [3, 4], [4, 2], [4, 3], [4, 4],
                          [3, 4], [3, 5], [3, 6], [4, 4], [4, 5], [4, 6], [5, 4], [5, 5], [5, 6]]
    goal_positions = [[7, 2], [7, 3]]

    def __init__(self, initial, goal=None):
        super().__init__(initial, goal)

    def check_valid(self, player_x, player_y, football_x, football_y):
        if 0 <= player_x < self.grid_width and 0 <= player_y < self.grid_height and \
                [player_x, player_y] not in self.opponents_player and \
                (player_x != football_x or player_y != football_y) and \
                0 <= football_x < self.grid_width and 0 <= football_y < self.grid_height and \
                [football_x, football_y] not in self.opponents_football:
            return True
        else:
            return False
    ...
    def successor(self, state):
        ...
        # player Up
        if self.check_valid(player_x, player_y + 1, football_x, football_y):
            successors["Pomesti coveche gore"] = (player_x, player_y + 1, football_x, football_y)
        # player Down
        if self.check_valid(player_x, player_y - 1, football_x, football_y):
            successors["Pomesti coveche dolu"] = (player_x, player_y - 1, football_x, football_y)
        # player Right
        if self.check_valid(player_x + 1, player_y, football_x, football_y):
            successors["Pomesti coveche desno"] = (player_x + 1, player_y, football_x, football_y)
        # player Up-Right
        if self.check_valid(player_x + 1, player_y + 1, football_x, football_y):
            successors["Pomesti coveche gore-desno"] = (player_x + 1, player_y + 1, football_x, football_y)
        # player Down-Right
        if self.check_valid(player_x + 1, player_y - 1, football_x, football_y):
            successors["Pomesti coveche dolu-desno"] = (player_x + 1, player_y - 1, football_x, football_y)

        # football Up
        if (player_x == football_x and player_y + 1 == football_y) and \
                self.check_valid(player_x, player_y + 1, football_x, football_y + 1):
            successors["Turni topka gore"] = (player_x, player_y + 1, football_x, football_y + 1)
        # football Down
        if (player_x == football_x and player_y - 1 == football_y) and \
                self.check_valid(player_x, player_y - 1, football_x, football_y - 1):
            successors["Turni topka dolu"] = (player_x, player_y - 1, football_x, football_y - 1)
        # football Right
        if (player_x + 1 == football_x and player_y == football_y) and \
                self.check_valid(player_x + 1, player_y, football_x + 1, football_y):
            successors["Turni topka desno"] = (player_x + 1, player_y, football_x + 1, football_y)
        # football Up-Right
        if (player_x + 1 == football_x and player_y + 1 == football_y) and \
                self.check_valid(player_x + 1, player_y + 1, football_x + 1, football_y + 1):
            successors["Turni topka gore-desno"] = (player_x + 1, player_y + 1, football_x + 1, football_y + 1)
        # football Down-Right
        if (player_x + 1 == football_x and player_y - 1 == football_y) and \
                self.check_valid(player_x + 1, player_y - 1, football_x + 1, football_y - 1):
            successors["Turni topka dolu-desno"] = (player_x + 1, player_y - 1, football_x + 1, football_y - 1)
    ...
    def goal_test(self, state):
        return [state[2], state[3]] in self.goal_positions


if __name__ == '__main__':
    # (player_x, player_y, football_x, football_y)
    player_input = input().split(',')
    football_input = input().split(',')
    initial_state = (int(player_input[0]), int(player_input[1]), int(football_input[0]), int(football_input[1]))
    player = Player(initial_state)
    print(breadth_first_graph_search(player).solution())
