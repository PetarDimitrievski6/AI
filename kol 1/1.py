...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        ghost_x = state[0]
        ghost_y = state[1]
        return 0 <= ghost_x < n and 0 <= ghost_y < n and \
            (ghost_x, ghost_y) not in walls
    ...
    def successor(self, state):
        ...
        for i in range(1, 4):
            # up
            if self.check_valid((ghost_x, ghost_y + i), self.walls, self.n):
                successors[f"Gore {i}"] = (ghost_x, ghost_y + i)

            # right
            if self.check_valid((ghost_x + i, ghost_y), self.walls, self.n):
                successors[f"Desno {i}"] = (ghost_x + i, ghost_y)
    ...
    def h(self, node):
        ...
        pacman_x = self.n - 1
        pacman_y = self.n - 1
        result = (abs(ghost_x - pacman_x) + abs(ghost_y - pacman_y)) / 3
        return result
